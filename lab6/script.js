...
function validateSizeByLimitations(n) {
    n = parseInt(n);
    return n > 0 && n <= 50
}
...
function removeDimensionsForWeightLessThan(weight) {
    ...
    let tbody = getNewTbody(hasAtLeastOneVolume(details));
    details.forEach(detail => tbody.appendChild(generateRowByDetail(detail)));
    clearInput(weightInputId);
}
